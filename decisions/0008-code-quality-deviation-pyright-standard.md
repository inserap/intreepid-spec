...
> Déviation **appliquée dès maintenant** (la config `src` est live) ; le statut
> `Proposed` reste cohérent avec le flux d'acceptation du repo (cf. Q-0013,
> aucune ADR encore promue `Accepted`).

## Context

`standards@0.7.0` introduit le gate qualité obligatoire pour tout impl repo
(`<STANDARDS>/conventions/code-quality.md`) : `ruff format` + `ruff check`
...
1. **`src` dévie à `typeCheckingMode="standard"`** (et non `strict`) dans son
   `pyproject.toml`, déclaré via le mécanisme de `<STANDARDS>/conventions/README.md`
   § override. `ruff` (E,F,I,UP,B + D restreint à l'en-tête de module) et `pytest`
...
- Doctrine : `<STANDARDS>/conventions/code-quality.md` § « Reference config » et
  § « Per-project deviation » ; `<STANDARDS>/conventions/README.md` § « How to
  override per project ».
- Config appliquée : `<IMPL:src>/pyproject.toml` (`[tool.pyright] typeCheckingMode="standard"`).
